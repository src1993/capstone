...
# demand_all = read.csv("data/created/demand_m1.csv",header=TRUE)
# 
...
# demand=read.csv("data/created/demand.csv",header=TRUE,stringsAsFactors = FALSE)
...
#   geom_rect(data=events,aes(xmin=start, xmax=end, ymin=0, ymax=3500000),alpha=0.3)
# 
# 
...
# 
# plot_every_year+
#   geom_rect(data=events,aes(xmin=start, xmax=end, ymin=0, ymax=20000, group=EVENT),alpha=0.3)


#colorful with events

# plot_every_year+
#   geom_rect(data=events,aes(xmin=start, xmax=end, ymin=0, ymax=20000, group=EVENT, fill=EVENT),alpha=0.2)
...
# 
# plot_every_year2+
#   geom_rect(data=events,aes(xmin=start, xmax=end, ymin=0, ymax=10000, group=EVENT),alpha=0.3)
# 


#find out the most popular 6 SKU to see their sale patterns
# sku_sale=demand_all%>%group_by(SKU_IDNT,dept)%>%
...
#   )
# 
# plot_price_type+geom_rect(data=events,aes(xmin=start, xmax=end, ymin=0, ymax=15000, group=EVENT),alpha=0.3)
...
ggplot(style_demand, aes(x=style, y=units_style, fill=style)) + geom_boxplot() + coord_flip()
